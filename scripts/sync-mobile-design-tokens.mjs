...
const nativeTokensPath = join(root, "packages/mobile-ui/src/lib/design-tokens.ts");
...
const palette = (tokens) =>
  Object.fromEntries([
    ["background", tokens.get("--ds-surface-sunken")],
    ["surface", tokens.get("--ds-surface")],
    ["text", tokens.get("--ds-text")],
    ["subtle", tokens.get("--ds-text-subtle")],
    ["border", tokens.get("--ds-border")],
    ["primary", tokens.get("--ds-link")],
    ["selected", tokens.get("--ds-background-selected")]
  ]);
const nativeTokensSource =
  "// Generated by scripts/sync-mobile-design-tokens.mjs. Do not edit.\n" +
  "export const nativeThemeTokens = " +
  JSON.stringify(
    {
      light: palette(lightFunctional),
      dark: palette(new Map([...lightFunctional, ...darkFunctional]))
    },
    null,
    2
  ) +
  ...
  JSON.stringify(
    Object.fromEntries(
      ["control", "overlay", "drawer"].map((name) => [
        name,
        Number.parseInt(rootVars.get("--ds-motion-" + name), 10)
      ])
    ),
    null,
    2
...
const nativeTokens = await format(nativeTokensSource, {
  ...(await resolveConfig(nativeTokensPath)),
  parser: "typescript"
});

if (checkOnly) {
  if (readFileSync(nativeTokensPath, "utf8").replaceAll("\r\n", "\n") !== nativeTokens) {
    console.error("Native runtime tokens are out of sync. Run pnpm tokens:mobile.");
    process.exit(1);
  }
...
writeFileSync(nativeTokensPath, nativeTokens);
