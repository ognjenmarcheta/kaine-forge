...
| Lockfile-only or trivial patch/minor, clean merge, real green CI                                              | Ensure `release:skip-changeset` when the bump touches `apps/**`/`packages/**` without a product release; **merge** if user asked to resolve the queue        |
...
| Test-runtime majors (jsdom, etc.)                                                                             | Merge only with package tests green; else dedicated PR or close with reason                                                                                  |
...
For each PR: `status` (`merged` / `recreated+merged` / `closed` / `left-open`) + one-line reason.
