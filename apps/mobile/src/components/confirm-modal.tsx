  ...
  return (
    <Modal animationType="fade" transparent visible={isOpen}>
      <View className="flex-1 items-center justify-center bg-black/40 px-5">
        <View className="w-full max-w-md rounded-2xl bg-background-surface p-4">
          <Text className="text-lg font-semibold text-text-default">{title}</Text>
          <Text className="mt-2 text-sm text-text-subtle">{message}</Text>
          <View className="mt-4 flex-row justify-end gap-2">
            <Pressable
              ...
              onPress={onCancel}
            >
              <Text className="text-sm text-text-default">{cancelLabel}</Text>
            </Pressable>
            <Pressable
              className="rounded-md bg-background-danger px-3 py-2"
              disabled={isConfirming}
              onPress={onConfirm}
            >
