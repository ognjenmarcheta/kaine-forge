...
If `CONTEXT.md` is missing in a downstream copy of this template, proceed silently. Create it when terms or decisions first need to be recorded.
...
If the concept needed is not in the glossary yet, either reconsider whether the term is project language or record the gap in `CONTEXT.md`.

## Flag ADR conflicts

If output contradicts an existing ADR, surface it explicitly rather than silently overriding it:

> _Contradicts ADR 0001 (Tauri over Electron) — but worth reopening because..._
